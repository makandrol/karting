...
scp -i ~/.ssh/id_github collector/src/index.js collector/src/storage.js \
  ubuntu@141.147.32.196:/home/ubuntu/collector/src/

# Якщо тільки один файл
scp -i ~/.ssh/id_github collector/src/storage.js \
  ubuntu@141.147.32.196:/home/ubuntu/collector/src/storage.js

# Якщо змінився package.json (нова залежність)
scp -i ~/.ssh/id_github collector/package.json \
  ubuntu@141.147.32.196:/home/ubuntu/collector/package.json
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "cd /home/ubuntu/collector && npm install"
```

4. **Restart PM2**:
```bash
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 restart collector"
```

5. **Verify health** — це обов'язково, не пропускай:
```bash
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "curl -s localhost:3001/healthz"
```
...
```bash
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 logs collector --nostream --lines 20"
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 logs collector --nostream --lines 10 --err"
```
...
```bash
# PM2 list
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 list"

# Stop / start
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 stop collector"
ssh -i ~/.ssh/id_github ubuntu@141.147.32.196 "pm2 start collector"

# DB stats / sessions count
curl http://141.147.32.196:3001/status

# Перевірка живих даних
curl http://141.147.32.196:3001/timing
```
