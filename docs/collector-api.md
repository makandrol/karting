...
## Live competition detector (auto-detection)

| Method | Path | Опис |
|---|---|---|
| GET | `/competition` | Поточний detector state |
| POST 🔒 | `/competition/start` | `{format, name}` — manual start |
| POST 🔒 | `/competition/stop` | Stop |
| POST 🔒 | `/competition/phase` | `{sessionId, type, name}` — позначити фазу |
| POST 🔒 | `/competition/reset` | Скинути today |
